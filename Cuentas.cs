...
namespace ConsolaEncriptacionEstandar
{
    public partial class Cuentas : Form
    {
        ...
        MCuentas metodosCuentas = new MCuentas();
        ...
        {
            ////**** EJEMPLO ****////
            //funcionesAuxiliares.llenarGrid(dtgCuentas,
            //    metodosCuentas.listar());
        ...
        private void dtgCuentas_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                if (this.dtgCuentas.Columns[e.ColumnIndex].Name == "Eliminar")
                {
                ...
                if (this.dtgCuentas.Columns[e.ColumnIndex].Name == "Modificar")
                {
        ...
        private void dtgCuentas_CellPainting(object sender, DataGridViewCellPaintingEventArgs e)
        {
           
            if (e.ColumnIndex >= 0 && this.dtgCuentas.Columns[e.ColumnIndex].Name == "Eliminar" && e.RowIndex >= 0)
            {
                e.Paint(e.CellBounds, DataGridViewPaintParts.All);

                DataGridViewButtonCell celBoton = this.dtgCuentas.Rows[e.RowIndex].Cells["Eliminar"] as DataGridViewButtonCell;
                ...
                this.dtgCuentas.Rows[e.RowIndex].Height = icoAtomico.Height + 8;
                this.dtgCuentas.Columns[e.ColumnIndex].Width = icoAtomico.Width + 8;

                e.Handled = true;

            }
            if (e.ColumnIndex >= 0 && this.dtgCuentas.Columns[e.ColumnIndex].Name == "Modificar" && e.RowIndex >= 0)
            {
                e.Paint(e.CellBounds, DataGridViewPaintParts.All);

                DataGridViewButtonCell celBoton = this.dtgCuentas.Rows[e.RowIndex].Cells["Modificar"] as DataGridViewButtonCell;
